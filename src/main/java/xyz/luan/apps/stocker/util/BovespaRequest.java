...
public class BovespaRequest {
    ...
    public double getCurrentValue(String stock) throws InvalidPaperException {
        try {
    ...
    private double extractValueFromResponseEntity(HttpEntity entity) throws JDOMException, IOException, InvalidPaperException {
        ...
        if (result.size() == 0) {
            throw new InvalidPaperException();
        }
        return convertToDouble(result.get(0).getAttribute("Ultimo").getValue());
    }
    ...
    public static class InvalidPaperException extends Exception {
        private static final long serialVersionUID = 1016151059130908433L;
    }
}
